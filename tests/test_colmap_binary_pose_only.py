...
def test_binary_and_text_pose_parsers_match(tmp_path: Path) -> None:
    records = _sample_pose_records()
    model_dir = tmp_path / "binary"
    model_dir.mkdir()
    _write_cameras_bin(model_dir / "cameras.bin")
    _write_images_bin(model_dir / "images.bin", records)

    text_path = tmp_path / "images.txt"
    _write_images_txt(text_path, records)

    detector = GapDetector()
    text_cameras = detector.parse_colmap_images(str(text_path))
    binary_cameras = detector.parse_colmap_binary_poses(str(model_dir))

    assert len(text_cameras) == len(binary_cameras) == len(records)

    text_by_id = {cam.image_id: cam for cam in text_cameras}
    binary_by_id = {cam.image_id: cam for cam in binary_cameras}

    for image_id in text_by_id:
        text_cam = text_by_id[image_id]
        binary_cam = binary_by_id[image_id]
        assert text_cam.name == binary_cam.name
        assert text_cam.camera_id == binary_cam.camera_id
        np.testing.assert_allclose(text_cam.position, binary_cam.position, atol=1e-9)
        np.testing.assert_allclose(text_cam.qvec, binary_cam.qvec, atol=1e-9)
        np.testing.assert_allclose(text_cam.tvec, binary_cam.tvec, atol=1e-9)
        assert text_cam.timestamp == binary_cam.timestamp
